                    ...
                    <form @submit="removeUserToGroup">
                        ...
                        <select class="form-control form-control-sm" @change="getSelectedGroup($event)">
                            <option selected disabled>Chose one user</option>
                             <option v-for="user in users" :key="user.name" @value="user">{{user}}</option>
                        </select>
...
export default {
    ...
    methods: {
        ...
        await axios.get(`http://localhost:8000/api/internations/groups/users/${group_id}`)
            .then(({data})=>{
                this.users = data       
            });
        },
        async removeUserToGroup(){
            await axios.put('http://localhost:8000/api/internations/groups/edit_users', {
